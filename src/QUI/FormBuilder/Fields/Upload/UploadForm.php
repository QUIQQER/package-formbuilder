...
class UploadForm extends Form
{
     ...
     * @var string[]
     */
    public static $fileEndingToMimeType = [
            ...
            if (!empty($fileParams['projectname']) && \is_string($fileParams['projectname']) &&
                ...
                try {
                    ...
                    if ($UploadField) {
                        ...
                        if (!empty($UploadField->getAttribute('file_types'))) {
                            $fileTypes = \array_merge($fileTypes, $UploadField->getAttribute('file_types'));
                        }

                        $params['maxFileSize'] = (int)$UploadField->getAttribute('file_size');

                        $fileTypesCustom = $UploadField->getAttribute('file_endings_custom');
                        ...
                        if (!empty($fileTypesCustom)) {
                            ...
                            foreach ($fileTypesCustom as $k => $v) {
                                ...
                                if (isset(self::$fileEndingToMimeType[$fileEnding])) {
                                    $fileTypes[] = self::$fileEndingToMimeType[$fileEnding];
                                }
                            }
                        }

                        $params['allowedFileTypes'] = \array_values(\array_unique($fileTypes));
                    }
    ...
    public function onFileFinish($file, $params)
    {
        ...
        // Validate token
        $token = Orthos::clear($uploadParams['upload_csrf_token']);
